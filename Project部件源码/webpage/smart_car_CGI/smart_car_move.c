...
{	
	char *cspeed="7000";
	//set directions
    fd = open("/sys/class/mydriver/my_device/direction1", O_WRONLY|O_TRUNC);
    if (fd < 0)
        printf("can't open value\n");
    write(fd, "0", 1);
    close(fd);
	
	fd = open("/sys/class/mydriver/my_device/direction2", O_WRONLY|O_TRUNC);
    if (fd < 0)
        printf("can't open value\n");
    write(fd, "0", 1);
    close(fd);
	
	fd = open("/sys/class/mydriver/my_device/direction3", O_WRONLY|O_TRUNC);
    if (fd < 0)
        printf("can't open value\n");
    write(fd, "1", 1);
    close(fd);
	
	fd = open("/sys/class/mydriver/my_device/direction4", O_WRONLY|O_TRUNC);
    if (fd < 0)
        printf("can't open value\n");
    write(fd, "1", 1);
    close(fd);
